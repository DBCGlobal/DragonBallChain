 ...
 * (http://www.DragonBallChain.com), which enables instant payments to anyone, anywhere in the world. WICC uses peer-to-peer
...
bool AppInit(int argc, char* argv[], boost::thread_group& threadGroup) {
    ...
    try {
        ...
        if (SysCfg().IsArgCount("-?") || SysCfg().IsArgCount("--help")) {
            ...
            std::string strUsage = _("DragonBallChain Coin Daemon") + " " + _("version") + " " + FormatFullVersion() +
                                   "\n\n" + _("Usage:") + "\n" + "  coind [options]                     " +
